...
INSERT INTO `__new_trusted_submissions`("id", "assignment_id", "user_id", "client_submission_id", "request_hash", "source_hash", "status", "verdict", "result_json", "settlement_hash", "submitted_at", "settled_at", "purge_after") SELECT "id", "assignment_id", "user_id", "client_submission_id", "request_hash", "source_hash", "status", "verdict", "result_json", CASE WHEN "status" = 'settled' THEN "request_hash" ELSE NULL END, "submitted_at", "settled_at", "purge_after" FROM `trusted_submissions`;--> statement-breakpoint
