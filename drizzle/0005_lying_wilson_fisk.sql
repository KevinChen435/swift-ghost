...
CREATE TABLE `__new_trusted_submissions` (
	...
	CONSTRAINT "trusted_submissions_verdict_check" CHECK("__new_trusted_submissions"."verdict" IS NULL OR "__new_trusted_submissions"."verdict" IN ('accepted', 'wrong-answer', 'compile-error', 'runtime-error', 'time-limit', 'judge-error')),
...
INSERT INTO `__new_trusted_submissions`("id", "assignment_id", "user_id", "client_submission_id", "request_hash", "source_hash", "status", "verdict", "result_json", "settlement_hash", "submitted_at", "enqueued_at", "settled_at", "purge_after") SELECT "id", "assignment_id", "user_id", "client_submission_id", "request_hash", "source_hash", "status", "verdict", "result_json", "settlement_hash", "submitted_at", "enqueued_at", "settled_at", "purge_after" FROM `trusted_submissions`;--> statement-breakpoint
