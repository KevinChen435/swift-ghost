...
export const PYTHON_PROBLEMS: PythonProblem[] = [
  {
    ...
    summary: "Count normalized words and return the most common word with a deterministic tie break.",
    cue: "Translate each input once, then update one dictionary entry.",
    invariant: "After processing index i, counts stores the exact frequency of every normalized word through i.",
    complexity: "O(n) expected time · O(k) space",
    languageNote: "dict.get(key, 0) is the compact interview-safe way to increment a possibly missing key.",
    estimatedMinutes: 4,
  ...
  },
  {
    ...
    difficulty: "Easy",
    pattern: "Python Fluency",
    summary: "Remove duplicates while preserving the first occurrence of each value.",
    ...
    complexity: "O(n) expected time · O(k) space",
    languageNote: "A set gives expected O(1) membership, while list order stays under your control.",
    estimatedMinutes: 3,
  ...
  },
  {
    ...
    invariant: "Every emitted tuple describes one previously visited position whose values differ.",
    complexity: "O(n) time · O(m) output space",
    languageNote: "enumerate(zip(left, right)) replaces manual index bookkeeping and unpacks cleanly in the loop target.",
    estimatedMinutes: 3,
  ...
  },
  {
    ...
    difficulty: "Easy",
    pattern: "Python Fluency",
    summary: "Rank records by descending score, then ascending name, without mutating the input.",
    cue: "Represent every tie-break rule in one tuple key.",
    invariant: "The returned records are ordered by (-score, normalized name), while the input list is unchanged.",
    complexity: "O(n log n) time · O(n) space",
    languageNote: "sorted returns a new list; negate numeric fields that must sort in descending order.",
    estimatedMinutes: 4,
  ...
  },
  {
    ...
    difficulty: "Easy",
    pattern: "Python Fluency",
    summary: "Traverse a small dependency graph in breadth-first order from one starting node.",
    cue: "Append new work on the right and consume old work from the left.",
    invariant: "The deque contains discovered but unprocessed nodes in discovery order.",
    complexity: "O(V + E) time · O(V) space",
    languageNote: "collections.deque.popleft is O(1); list.pop(0) shifts the remaining list.",
    estimatedMinutes: 5,
...
def breadth_first_order(graph: dict[str, list[str]], start: str) -> list[str]:
  ...
  },
  {
    ...
    difficulty: "Easy",
    pattern: "Python Fluency",
    summary: "Return the next tasks ordered by priority, then submission sequence.",
    cue: "Put every comparison field before the payload in a heap tuple.",
    invariant: "The heap root is the remaining task with the smallest (priority, sequence) pair.",
    complexity: "O(n + k log n) time · O(n) space",
    languageNote: "heapq is a min-heap; a sequence field prevents Python from comparing arbitrary payloads when priorities tie.",
    estimatedMinutes: 5,
    code: `import heapq
...
def next_tasks(tasks: list[tuple[int, int, str]], limit: int) -> list[str]:
  ...
  },
  {
    ...
    difficulty: "Easy",
    pattern: "Python Fluency",
    summary: "Normalize valid scores and compute their squared total without building an unnecessary second list.",
    cue: "Use a comprehension for reusable data and a generator for one-pass aggregation.",
    invariant: "normalized contains only in-range integer scores, in original order.",
    complexity: "O(n) time · O(n) space for normalized values",
    languageNote: "sum(expression for item in items) consumes a generator lazily; square brackets would allocate another list.",
    estimatedMinutes: 4,
  ...
  },
  {
    ...
    invariant: "After each event, both structures exactly summarize the processed prefix.",
    complexity: "O(n) expected time · O(n) space",
    languageNote: "defaultdict(list) creates a fresh list per missing key; Counter communicates frequency intent directly.",
    estimatedMinutes: 5,
...
def summarize_events(
  ...
  },
  {
    ...
    invariant: "index_by_value contains only values from indices strictly before the current index.",
    complexity: "O(n) expected time · O(n) space",
    languageNote: "Use `if complement in dictionary` when index zero is a valid stored answer; truthiness would be incorrect.",
    estimatedMinutes: 5,
    code: `class Solution:
  ...
  },
  {
    ...
    difficulty: "Medium",
    pattern: "Arrays & Hashing",
    summary: "Group words that contain the same characters with the same multiplicities.",
    cue: "Turn each word into an immutable canonical frequency signature.",
    invariant: "Every word under a signature has exactly the 26 counts represented by that signature.",
    complexity: "O(nk) time · O(nk) space",
    languageNote: "A tuple is hashable and can serve as a dictionary key; a list cannot.",
    estimatedMinutes: 9,
...
class Solution:
  ...
  },
  {
    ...
    invariant: "Before the reverse pass reaches i, answer[i] equals the product strictly left of i.",
    complexity: "O(n) time · O(1) auxiliary space excluding output",
    languageNote: "range(len(nums) - 1, -1, -1) is the standard explicit reverse-index loop.",
    estimatedMinutes: 9,
    code: `class Solution:
  ...
  },
  {
    ...
    summary: "Decide whether text reads the same forward and backward after ignoring punctuation and case.",
    cue: "Advance inward from both ends, skipping characters outside the comparison alphabet.",
    invariant: "Every relevant character outside [left, right] has already matched its mirror.",
    complexity: "O(n) time · O(1) auxiliary space",
    languageNote: "str.isalnum and str.lower keep the pointer solution readable without allocating a normalized copy.",
    estimatedMinutes: 6,
    code: `class Solution:
  ...
  },
  {
    ...
    invariant: "Duplicate fixed and pointer values are skipped, so each emitted value triple is unique.",
    complexity: "O(n²) time · O(n) sorting space excluding output",
    languageNote: "Python's in-place list.sort is concise here because the input need not be preserved by the judge.",
    estimatedMinutes: 13,
    code: `class Solution:
  ...
  },
  {
    ...
    summary: "Find the maximum length of a contiguous substring with distinct characters.",
    cue: "A repeated character matters only when its last position is inside the current window.",
    invariant: "The window from left through right has no duplicate characters.",
    complexity: "O(n) time · O(k) space",
    languageNote: "max(left, last_seen[character] + 1) prevents the left boundary from moving backward.",
    estimatedMinutes: 8,
    code: `class Solution:
  ...
  },
  {
    ...
    difficulty: "Medium",
    pattern: "Sliding Window",
    summary: "Find the shortest substring containing every required character with multiplicity.",
    cue: "Expand until all requirements are satisfied, then shrink while they remain satisfied.",
    invariant: "formed counts how many required character kinds currently meet their exact needed frequency.",
    complexity: "O(|s| + |t|) time · O(k) space",
    languageNote: "Counter captures multiplicity; defaultdict-free window updates stay explicit during shrink and expand.",
    estimatedMinutes: 15,
...
class Solution:
  ...
  },
  {
    ...
    difficulty: "Easy",
    pattern: "Stack",
    summary: "Check that every closer matches the most recent unmatched opener.",
    cue: "Nested delimiters require last-in, first-out matching.",
    invariant: "The stack contains exactly the unmatched opening delimiters in encounter order.",
    complexity: "O(n) time · O(n) space",
    languageNote: "list.append and list.pop provide an efficient stack at the right end of a Python list.",
    estimatedMinutes: 5,
    code: `class Solution:
  ...
  },
  {
    ...
    invariant: "Temperatures at stack indices are non-increasing from bottom to top.",
    complexity: "O(n) time · O(n) space",
    languageNote: "Store indices rather than temperatures so one stack value supports both comparison and distance.",
    estimatedMinutes: 8,
    code: `class Solution:
  ...
  },
  {
    ...
    languageNote: "Use // for integer midpoint arithmetic; / produces a float in Python.",
    estimatedMinutes: 5,
    code: `class Solution:
  ...
  },
  {
    ...
    difficulty: "Medium",
    pattern: "Binary Search",
    summary: "Find the smallest integer speed that finishes all piles within the hour limit.",
    cue: "Feasibility is monotonic: every speed above a feasible speed is also feasible.",
    invariant: "The minimum feasible speed remains inside the closed range [left, right].",
    complexity: "O(n log m) time · O(1) space, where m is the largest pile",
    languageNote: "(pile + speed - 1) // speed computes positive ceiling division without floating point.",
    estimatedMinutes: 10,
    code: `class Solution:
  ...
  },
  {
    ...
    invariant: "previous heads the fully reversed processed prefix; current heads the untouched suffix.",
    complexity: "O(n) time · O(1) space",
    languageNote: "Python tuple assignment can express the pointer rotation, but explicit steps make the mutation order easier to explain.",
    estimatedMinutes: 6,
    code: `class Solution:
  ...
  },
  {
    ...
    invariant: "tail ends a sorted merged prefix containing every consumed node.",
    complexity: "O(n + m) time · O(1) space",
    languageNote: "Reusing existing nodes avoids allocations; only the sentinel node is new.",
    estimatedMinutes: 6,
    code: `class Solution:
    def mergeTwoLists(
  ...
  },
  {
    ...
    invariant: "Before they meet, slow advances one edge and fast advances two edges per round.",
    complexity: "O(n) time · O(1) space",
    languageNote: "Use `is` for node identity; equal values do not imply the same linked-list node.",
    estimatedMinutes: 6,
    code: `class Solution:
  ...
  },
  {
    ...
    invariant: "Every stack pair contains a discovered node and its exact root-to-node depth.",
    complexity: "O(n) time · O(h) stack space",
    languageNote: "An explicit list stack keeps the DFS safe even when a valid skewed tree exceeds Python's recursion limit.",
    estimatedMinutes: 5,
    code: `class Solution:
  ...
  },
  {
    ...
    difficulty: "Medium",
    pattern: "Trees",
    summary: "Check that every node obeys all ancestor bounds of a binary search tree.",
    cue: "Carry an open lower and upper bound into each subtree.",
    invariant: "Every stack entry carries all ancestor bounds that its node must satisfy.",
    complexity: "O(n) time · O(h) stack space",
    languageNote: "An explicit stack avoids recursion-depth failures; float infinities make the initial open bounds concise.",
    estimatedMinutes: 9,
    code: `class Solution:
  ...
  },
  {
    ...
    invariant: "At the start of an outer iteration, the queue contains exactly one tree level.",
    complexity: "O(n) time · O(w) space",
    languageNote: "deque.popleft keeps breadth-first traversal linear; a list queue with pop(0) would not.",
    estimatedMinutes: 7,
    code: `from collections import deque
...
class Solution:
  ...
  },
  {
    ...
    invariant: "After each value, the heap contains the largest min(k, processed count) values seen so far.",
    complexity: "O(n log k) time · O(k) space",
    languageNote: "heapq.heapreplace removes the root and pushes the stronger candidate in one guarded operation.",
    estimatedMinutes: 8,
    code: `import heapq
...
class Solution:
  ...
  },
  {
    ...
    invariant: "The heap contains the k highest-frequency candidates among values processed so far when its size reaches k.",
    complexity: "O(n + u log k) time · O(u + k) space",
    languageNote: "Counter.items supplies (value, frequency), so construct heap tuples in comparison-first order.",
    estimatedMinutes: 8,
    code: `from collections import Counter
import heapq
...
class Solution:
  ...
  },
  {
    ...
    invariant: "merged is sorted and disjoint, and covers every interval processed so far.",
    complexity: "O(n log n) time · O(n) output space",
    languageNote: "Sorting lists of two integers uses lexicographic order, which naturally sorts by start then end.",
    estimatedMinutes: 8,
    code: `class Solution:
  ...
  },
  {
    ...
    difficulty: "Medium",
    pattern: "Intervals",
    summary: "Insert one range into sorted disjoint ranges and merge its overlaps.",
    cue: "Process intervals in three phases: before, overlapping, and after.",
    invariant: "Before the final append phase, result is sorted and merged covers every overlap encountered.",
    complexity: "O(n) time · O(n) output space",
    languageNote: "Tuple unpacking keeps the mutable merged start/end separate from the input list.",
    estimatedMinutes: 9,
    code: `class Solution:
    def insert(
  ...
  },
  {
    ...
    invariant: "Every land cell changed to water belongs to exactly one already-counted island.",
    complexity: "O(rows · columns) time · O(rows · columns) worst-case worklist space",
    languageNote: "Mutating the grid serves as the visited set, while an explicit list stack avoids Python recursion-depth failures.",
    estimatedMinutes: 9,
    code: `class Solution:
  ...
  },
  {
    ...
    difficulty: "Medium",
    pattern: "Graphs",
    summary: "Determine whether all courses can be completed under prerequisite edges.",
    cue: "Repeatedly remove zero-indegree nodes; a cycle leaves some nodes unremoved.",
    invariant: "indegree[c] equals the number of c's prerequisites not yet processed.",
    complexity: "O(V + E) time · O(V + E) space",
    languageNote: "deque implements Kahn's algorithm cleanly, and list-of-lists is enough for integer-labeled vertices.",
    estimatedMinutes: 11,
    code: `from collections import deque
...
class Solution:
  ...
  },
  {
    ...
    summary: "Generate combinations of reusable candidates that total a target.",
    cue: "After choosing a reusable value, recurse from the same candidate index.",
    invariant: "path is nondecreasing by candidate index and sums to target minus remaining.",
    complexity: "Output-sensitive exponential time · O(target / min(candidate)) stack space",
    languageNote: "Append before recursion and pop afterward to reuse one mutable path safely.",
    estimatedMinutes: 10,
    code: `class Solution:
  ...
  },
  {
    ...
    difficulty: "Medium",
    pattern: "Backtracking",
    summary: "Determine whether adjacent grid cells can spell a word without reusing a cell.",
    cue: "Temporarily mark one cell, explore four choices, then restore it.",
    invariant: "Cells marked during a call are exactly the current word-prefix path and cannot be reused.",
    complexity: "O(rows · columns · 3^L) time · O(L) stack space",
    languageNote: "In-place marking avoids allocating a visited set for every starting position, but restoration is mandatory.",
    estimatedMinutes: 12,
    code: `class Solution:
  ...
  },
  {
    ...
    invariant: "farthest is the maximum index reachable using positions through the current index.",
    complexity: "O(n) time · O(1) space",
    languageNote: "enumerate exposes both the position and jump capacity without manual indexing.",
    estimatedMinutes: 7,
    code: `class Solution:
  ...
  },
  {
    ...
    invariant: "previous_two and previous_one store the ways to reach the two preceding step counts.",
    complexity: "O(n) time · O(1) space",
    languageNote: "Parallel assignment updates the rolling DP state without a temporary variable.",
    estimatedMinutes: 5,
    code: `class Solution:
  ...
  },
  {
    ...
    invariant: "previous_one is the best total for the processed prefix; previous_two is the best total before its final house.",
    complexity: "O(n) time · O(1) space",
    languageNote: "Tuple assignment makes the two-state recurrence compact while evaluating the right side from old values.",
    estimatedMinutes: 7,
    code: `class Solution:
