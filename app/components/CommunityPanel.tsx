...
export function CommunityPanel({
  ...
  useEffect(() => {
    function syncRoute() {
      ...
      if (nextTab === "recent" || nextTab === "daily" || nextTab === "records" || nextTab === "profile") setTab(nextTab);
      const nextHandle = params.get("profile")?.trim();
      setRequestedHandle(nextHandle ?? "");
      if (nextHandle) { setHandleLookup(nextHandle); if (!nextTab) setTab("profile"); }
  ...
  async function findProfile(event: FormEvent<HTMLFormElement>) {
    ...
    const url = new URL(window.location.href); url.searchParams.set("view", "records"); url.searchParams.set("tab", "profile"); url.searchParams.set("profile", requested); window.history.pushState({}, "", url);
  }

  function selectTab(value: CommunityTab) {
    setLoading(value !== "profile"); setTab(value);
    const url = new URL(window.location.href); url.searchParams.set("view", "records"); url.searchParams.set("tab", value); if (value !== "profile") url.searchParams.delete("profile"); window.history.pushState({}, "", url);
  }
  ...
  return <section className="community-shell">
    ...
    <div className="community-tabs" role="tablist" aria-label="Community sections">{(["recent", "daily", "records", "profile"] as CommunityTab[]).map((value) => <button key={value} role="tab" aria-selected={tab === value} className={tab === value ? "active" : ""} onClick={() => selectTab(value)}>{value === "records" ? "Leaderboards" : value[0].toUpperCase() + value.slice(1)}</button>)}</div>
