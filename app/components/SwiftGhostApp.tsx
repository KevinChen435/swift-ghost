...
import { INTERVIEW_PATTERN_ORDER, IOS_PATTERN_ORDER, PATTERN_ORDER, PYTHON_PATTERN_ORDER, problemLineCount, problemUrl, type Difficulty, type Pattern } from "../data/problems";
import { CommunityPanel } from "./CommunityPanel";
import { BUILTIN_ITEMS, itemDisplayId, makeCustomItem, updateCustomItem, type CodeLanguage, type ItemId, type PracticeItem } from "../lib/items";
import { buildSessionQueue, type SessionLanguage, type SessionQueueEntry, type SessionSource, type SessionStageMode, type SessionTrack } from "../lib/sessions.mjs";
import { parseRoute, resolveRouteItem, routeForItem, serializeRoute, type AppRoute } from "../lib/routes.mjs";
import { createCloudClient, type CloudCapabilities, type CloudDailyChallenge, type CloudSession } from "../lib/cloud.mjs";
import {
  EMPTY_STATE,
  FIRST_STORAGE_KEY,
  LEGACY_STORAGE_KEY,
...
type SessionBuildOptions = { name: string; count: number; source: SessionSource; track: SessionTrack; language: SessionLanguage; pattern: string; difficulty: string; stageMode: SessionStageMode };
type CloudRuntime = { status: "checking" | "local" | "signed-out" | "connected" | "syncing" | "error"; capabilities: CloudCapabilities | null; session: CloudSession | null; dailyChallenge: CloudDailyChallenge | null; refresh: number };
...
const LANGUAGE_META: Record<CodeLanguage, { label: string; short: string; file: string; note: string }> = {
  python: { label: "Python", short: "Py", file: "solution.py", note: "Python note" },
  swift: { label: "Swift", short: "S", file: "Solution.swift", note: "Swift note" },
};
...
function matchesLane(item: Pick<PracticeItem, "track" | "language">, value: "All" | "python" | "swift" | "ios") {
...
function freshDraft(itemId: ItemId, stage: number, itemRevision = 1, challengeDate?: string, sessionId?: string): Draft {
  return { itemId, itemRevision, stage, value: "", startedAt: null, totalKeystrokes: 0, correctKeystrokes: 0, rejectedKeystrokes: 0, corrections: 0, peeks: 0, keyErrors: {}, challengeDate, sessionId };
}
...
export default function SwiftGhostApp() {
  ...
  const [cloud, setCloud] = useState<CloudRuntime>({ status: "checking", capabilities: null, session: null, dailyChallenge: null, refresh: 0 });
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      const restoredItem = items.find((candidate) => candidate.itemId === restored.lastItemId) ?? BUILTIN_ITEMS[0];
      setState(restored);
      setView(route.view);
  ...
  useEffect(() => {
    ...
    async function connectCloud() {
      const capabilities = await cloudClient.capabilities({ signal: controller.signal });
      if (!capabilities.available) { setCloud((current) => ({ ...current, status: "local", capabilities: null, session: null, dailyChallenge: null })); return; }
      const utcToday = new Date().toISOString().slice(0, 10);
      const [session, daily] = await Promise.all([
        cloudClient.session({ signal: controller.signal }),
        cloudClient.dailyLeaderboard(utcToday, { limit: 1, signal: controller.signal }),
      ]);
      const dailyChallenge = daily.available ? daily.data.challenge : null;
      if (!session.available) { setCloud((current) => ({ ...current, status: session.reason === "unauthorized" ? "signed-out" : "error", capabilities: capabilities.data, session: null, dailyChallenge })); return; }
      setCloud((current) => ({ ...current, status: session.data.authenticated ? "connected" : "signed-out", capabilities: capabilities.data, session: session.data, dailyChallenge }));
    }
    void connectCloud();
  ...
  useEffect(() => {
    if (!ready || !state.cloud.communityEnabled || !cloud.session?.authenticated) return;
    const known = new Set(state.cloud.uploadedAttemptIds);
    const pending = state.attempts.filter((attempt) => attempt.outcome === "completed" && !attempt.itemId.startsWith("custom:") && !known.has(attempt.id)).slice(0, 50);
    if (!pending.length) return;
    const controller = new AbortController();
    void cloudClient.postAttemptBatch(pending.map((attempt) => {
      const matched = BUILTIN_ITEMS.find((candidate) => candidate.itemId === attempt.itemId);
      return { ...attempt, track: matched?.track, title: attempt.titleSnapshot, typedChars: matched?.code.length ?? attempt.correctKeystrokes, completed: true as const };
    }), { signal: controller.signal }).then((receipt) => {
      if (!receipt.available) { setCloud((current) => ({ ...current, status: receipt.reason === "aborted" ? "connected" : "error" })); return; }
      const settled = new Set([...receipt.data.accepted, ...receipt.data.duplicates, ...receipt.data.rejected.map((entry) => entry.id)]);
      mutateState((current) => ({ ...current, cloud: { ...current.cloud, uploadedAttemptIds: [...new Set([...current.cloud.uploadedAttemptIds, ...settled])].slice(-1000), lastSyncedAt: new Date().toISOString() } }));
      setCloud((current) => ({ ...current, status: "connected" }));
    });
    return () => controller.abort();
  }, [ready, state.cloud.communityEnabled, state.cloud.uploadedAttemptIds, state.attempts, cloud.session?.authenticated]);
  ...
  const ghostCode = maskCode(item.code, stage, reveal, item.masks, item.language);
  ...
  function createAttempt(active: Draft, activeItem: PracticeItem, outcome: AttemptRecord["outcome"], current: AppState) {
    ...
    const attempt: AttemptRecord = {
      id: makeId(), itemId: active.itemId, itemRevision: active.itemRevision, titleSnapshot: activeItem.title, language: activeItem.language, stage: active.stage,
      mode: active.challengeDate ? "strict" : current.settings.strictMode ? "strict" : "free",
  ...
  function openItem(next: PracticeItem, nextStage?: number, challengeDate?: string, sessionId?: string) {
    ...
    setSelectedId(next.itemId); setStage(chosenStage); setReveal(false); setResult(null); setView("practice"); writeRoute(routeForItem(next, chosenStage)); wpmSamples.current = [];
  ...
  function chooseStage(nextStage: number) {
    ...
    setStage(nextStage); setReveal(false); setResult(null); writeRoute(routeForItem(item, nextStage)); wpmSamples.current = [];
  ...
  function handleChange(event: ChangeEvent<HTMLTextAreaElement>) {
    ...
    const keyErrors = edit.inserted.split("").reduce((next, character, index) => {
      if (character === item.code[edit.prefix + index]) return next;
      const keyName = character === "\n" ? "↵" : character === " " ? "space" : character;
      return { ...next, [keyName]: (next[keyName] ?? 0) + 1 };
    }, { ...draft.keyErrors });
    if ((draft.challengeDate || state.settings.strictMode) && !correctPrefix && edit.insertedCount > 0) {
      const rejected = Math.max(1, edit.incorrectInserted || edit.insertedCount);
      updateDraft({ ...draft, keyErrors, startedAt, totalKeystrokes: draft.totalKeystrokes + edit.insertedCount, rejectedKeystrokes: draft.rejectedKeystrokes + rejected, corrections: draft.corrections + edit.deletedCount });
      setToast(`Expected ${JSON.stringify(item.code[edit.prefix] ?? "end of solution")}`);
      return;
    }
    const next: Draft = {
      ...draft, value: proposed, startedAt, keyErrors,
  ...
  function toggleCommunityUploads(enabled: boolean) { mutateState((current) => ({ ...current, cloud: { ...current.cloud, communityEnabled: enabled } })); setToast(enabled ? "Built-in attempt uploads enabled" : "Community uploads paused"); }
  ...
  function startSession(options: SessionBuildOptions, plannedEntries?: SessionQueueEntry[]) {
    ...
    const planned = plannedEntries?.filter((entry) => allItems.some((candidate) => candidate.itemId === entry.itemId && (options.track === "all" || candidate.track === options.track) && (options.language === "all" || candidate.language === options.language))).slice(0, 20);
    const entries = planned?.length ? planned : buildSessionQueue(allItems, signals, options);
    if (!entries.length) { setToast("No items match that session setup"); return; }
    const session: TrainingSession = { id: makeId(), name: options.name.trim() || "Practice session", source: options.source, track: options.track, language: options.language, stageMode: options.stageMode, createdAt: new Date().toISOString(), entries, currentIndex: 0 };
    mutateState((current) => {
  ...
  function skipSessionEntry() {
    ...
    if (nextIndex < 0) {
      ...
      setResult(null); navigateView("sessions"); setToast("Session finished"); return;
    }
  ...
  function endSession() {
    ...
    setResult(null); navigateView("sessions"); setToast("Session ended");
  }
  ...
  function saveCustom(input: Parameters<typeof makeCustomItem>[0]) {
    if (customEditor && customEditor !== "new") {
      ...
      const queuedInTrackSession = (updated.track !== customEditor.track && state.activeSession?.track !== "all" || updated.language !== customEditor.language && state.activeSession?.language !== "all") && state.activeSession?.entries.some((entry) => entry.itemId === customEditor.itemId && entry.status === "pending");
      if (queuedInTrackSession) { setToast("End the active track-specific session before moving this snippet"); return; }
      if (codeChanged && activeDraft && !window.confirm("The code changed. Save this edit and close the current draft? The old draft will be kept as an abandoned attempt.")) return;
      mutateState((current) => {
      ...
      setCustomEditor(null); setResult(null); setReveal(false); if (codeChanged) setStage(1);
  ...
  function archiveCustom(itemId: ItemId) {
    ...
    if (selectedId === itemId) { setSelectedId(BUILTIN_ITEMS[0].itemId); setStage(1); setReveal(false); setResult(null); }
  ...
  async function importProgress(event: ChangeEvent<HTMLInputElement>) {
    ...
    try { const parsed = JSON.parse(await file.text()) as unknown; const restored = normalizeState(parsed); if (!parsed || typeof parsed !== "object" || ![2, 3, 4, 5, 6, 7].includes(Number((parsed as { version?: unknown }).version))) throw new Error("invalid"); setState(restored); setSelectedId(restored.lastItemId); setStage(restored.lastStage); setReveal(false); setResult(null); wpmSamples.current = []; setToast("Progress restored and migrated"); } catch { setToast("That backup could not be read"); }
    event.target.value = "";
  }

  function resetAllData() {
    if (!window.confirm("Delete all Swift Ghost progress, custom snippets, and settings from this device?")) return;
    localStorage.removeItem(STORAGE_KEY); localStorage.removeItem(LEGACY_STORAGE_KEY); localStorage.removeItem(OLDER_STORAGE_KEY); localStorage.removeItem(OLDEST_STORAGE_KEY); localStorage.removeItem(ORIGINAL_STORAGE_KEY); localStorage.removeItem(FIRST_STORAGE_KEY); setState(EMPTY_STATE); setSelectedId(BUILTIN_ITEMS[0].itemId); setStage(1); setToast("Local data cleared");
  }
  ...
  function handleResultNext() {
    ...
    if (result.sessionComplete) { setResult(null); navigateView("sessions"); setToast("Session complete"); return; }
  ...
  return (
    <div className={`app-shell ${focusMode ? "is-focus" : ""}`}>
      <header className="topbar">
        <button className="brand" onClick={() => navigateView("today")} aria-label="Swift Ghost home"><span className="brand-mark" aria-hidden="true">S<span>G</span></span><span><strong>Swift Ghost</strong><small>type it · fade it · own it</small></span></button>
        <nav className="main-nav" aria-label="Main navigation">{NAV.map((nav) => <button key={nav.id} className={view === nav.id ? "active" : ""} aria-current={view === nav.id ? "page" : undefined} onClick={() => navigateView(nav.id)}><span aria-hidden="true">{nav.icon}</span>{nav.label}</button>)}</nav>
        <div className="top-actions"><button className="goal-pill" onClick={() => navigateView("today")} title="Today's practice goal"><span className="goal-ring" style={{ "--goal": `${dailyPercent * 3.6}deg` } as React.CSSProperties}>{dailyPercent}%</span><span><strong>{todayMinutes}/{state.settings.dailyGoalMinutes} min</strong><small>{activeStreak(state)} day streak</small></span></button><button className="icon-button" onClick={() => randomItem()} title="Random problem" aria-label="Open a random problem">↝</button></div>
      </header>

      {view === "today" && <TodayView ready={ready} state={state} items={allItems} cloudStatus={cloud.status} cloudDaily={cloud.dailyChallenge} onOpen={openItem} onReview={() => randomItem("due")} onBrowse={() => navigateView("library")} onCreate={() => setCustomEditor("new")} onSessions={() => navigateView("sessions")} />}
      {view === "practice" && <PracticeView state={state} items={allItems} item={item} draft={draft} stage={stage} metrics={metrics} ghostCode={ghostCode} stats={stats} dueCount={dueItems.length} reveal={reveal} focusMode={focusMode} errorKeys={draft.keyErrors} activeSession={state.activeSession} onOpenItem={openItem} onChooseStage={chooseStage} onChange={handleChange} onKeyDown={handleKeyDown} onPaste={(event) => { event.preventDefault(); const count = Math.max(1, event.clipboardData.getData("text").length); updateDraft({ ...draft, startedAt: draft.startedAt ?? Date.now(), totalKeystrokes: draft.totalKeystrokes + count, rejectedKeystrokes: draft.rejectedKeystrokes + count }); setToast("Pasting is disabled during a practice pass"); }} onReset={resetAttempt} onReveal={toggleReveal} onFavorite={() => toggleFavorite(selectedId)} onFocusMode={() => setFocusMode((value) => !value)} onReview={() => randomItem("due")} onBrowse={() => navigateView("library")} onSession={() => navigateView("sessions")} onSkipSession={skipSessionEntry} onEndSession={endSession} />}
      ...
      {view === "records" && <RecordsView key={`${cloud.status}:${cloud.refresh}:${cloud.session?.profile?.handle ?? "local"}:${cloud.session?.profile?.updatedAt ?? "new"}`} state={state} items={allItems} cloud={cloud} onOpen={openItem} onReview={() => randomItem("due")} onToggleUploads={toggleCommunityUploads} onCloudRefresh={() => setCloud((current) => ({ ...current, refresh: current.refresh + 1 }))} />}
...
function TodayView({ ready, state, items, cloudStatus, cloudDaily, onOpen, onReview, onBrowse, onCreate, onSessions }: { ready: boolean; state: AppState; items: PracticeItem[]; cloudStatus: CloudRuntime["status"]; cloudDaily: CloudDailyChallenge | null; onOpen: (item: PracticeItem, stage?: number, challengeDate?: string, sessionId?: string) => void; onReview: () => void; onBrowse: () => void; onCreate: () => void; onSessions: () => void }) {
  const todayDate = ready ? new Date() : new Date(2000, 0, 1, 12); const today = cloudDaily?.date ?? dayKey(todayDate); const interviewItems = BUILTIN_ITEMS.filter((item) => item.track === "interview"); const preferredInterviewItems = interviewItems.filter((item) => item.language === state.settings.preferredLanguage && item.pattern !== "Python Fluency"); const iosItems = BUILTIN_ITEMS.filter((item) => item.track === "ios"); const pythonFluency = BUILTIN_ITEMS.filter((item) => item.language === "python" && item.pattern === "Python Fluency"); const remoteDaily = cloudDaily ? interviewItems.find((item) => item.itemId === cloudDaily.itemId && item.contentRevision === cloudDaily.itemRevision) : undefined; const daily = remoteDaily ?? dailyItem(preferredInterviewItems.length ? preferredInterviewItems : interviewItems, todayDate); const dailyAvailable = cloudStatus === "local" || Boolean(cloudDaily && remoteDaily); const iosDaily = dailyItem(iosItems, new Date(todayDate.getTime() + 86400000)); const pythonDaily = dailyItem(pythonFluency, new Date(todayDate.getTime() + 43200000)); const due = items.filter((item) => isReviewDue(state, item.itemId));
  ...
  return <main className="page-container today-page">
    <PageHeading eyebrow={ready ? new Intl.DateTimeFormat(undefined, { weekday: "long", month: "long", day: "numeric" }).format(todayDate) : "Today"} title="Build recall, one clean pass at a time." copy="Reactivate Python for interviews, keep Swift and iOS sharp, and return to each solution on a spaced schedule." />
    <section className="today-hero">
      <div className="today-copy"><span className="eyebrow">Daily Type {dailyDone ? "· complete" : dailyAvailable ? "· ready" : "· connecting"}</span><h2>{daily?.title}</h2><p>{daily?.cue}</p><div className="problem-tags"><span className={`difficulty ${daily?.difficulty.toLowerCase()}`}>{daily?.difficulty}</span><span>{daily && LANGUAGE_META[daily.language].label}</span><span>{daily?.pattern}</span><span>Fixed stage 1</span><span>{daily ? problemLineCount(daily) : 0} lines</span></div><button className="primary-button" disabled={!daily || !dailyAvailable} onClick={() => daily && dailyAvailable && onOpen(daily, 1, today)}>{dailyDone ? "Practice it again" : dailyAvailable ? "Start today's benchmark" : cloudStatus === "checking" ? "Checking today's benchmark" : "Benchmark temporarily unavailable"}<span>→</span></button></div>
    ...
    <div className="today-grid">
      ...
      <article className="today-card priority"><span className="eyebrow">Python reactivation</span><h3>{pythonDaily?.title ?? "Python fundamentals"}</h3><p>{pythonDaily?.cue ?? "Short syntax and standard-library drills will appear here."}</p><button className="outline-button" disabled={!pythonDaily} onClick={() => pythonDaily && onOpen(pythonDaily)}>Warm up Python fluency →</button></article>
      <article className="today-card ios-reactivation"><span className="eyebrow">iOS reactivation</span><h3>{iosDaily?.title ?? "iOS fundamentals"}</h3><p>{iosDaily?.cue ?? "Short Swift and platform exercises will appear here."}</p><button className="outline-button" disabled={!iosDaily} onClick={() => iosDaily && onOpen(iosDaily)}>Practice this fundamental →</button></article>
      ...
      <article className="today-card"><span className="eyebrow">Focused set</span><h3>Build a deliberate session.</h3><p>Queue due work, new problems, favorites, or custom Python and Swift snippets with a fixed recall policy.</p><div className="card-actions"><button className="outline-button" onClick={onSessions}>Build session</button><button className="outline-button" onClick={onCreate}>Add snippet</button><button className="outline-button" onClick={onBrowse}>Library</button></div></article>
    </div>
  </main>;
}

type PracticeProps = {
  state: AppState; items: PracticeItem[]; item: PracticeItem; draft: Draft; stage: number;
  metrics: ReturnType<typeof currentMetrics>; ghostCode: string; stats: ReturnType<typeof itemStats>;
  dueCount: number; reveal: boolean; focusMode: boolean; errorKeys: Record<string, number>;
  activeSession: TrainingSession | null; onOpenItem: (item: PracticeItem, stage?: number) => void;
  onChooseStage: (stage: number) => void; onChange: (event: ChangeEvent<HTMLTextAreaElement>) => void;
  onKeyDown: (event: KeyboardEvent<HTMLTextAreaElement>) => void; onPaste: (event: ReactClipboardEvent<HTMLTextAreaElement>) => void;
  onReset: () => void; onReveal: () => void; onFavorite: () => void;
  onFocusMode: () => void; onReview: () => void; onBrowse: () => void; onSession: () => void;
  onSkipSession: () => void; onEndSession: () => void;
};
...
function PracticeView(props: PracticeProps) {
  ...
  return <main className="practice-layout">
    ...
    <section className="practice-main">
      ...
      <div className="problem-header"><div><div className="problem-kicker"><span>{itemDisplayId(props.item)}</span><span className={`difficulty ${props.item.difficulty.toLowerCase()}`}>{props.item.difficulty}</span><span>{laneLabel(props.item)}</span><span>{props.item.pattern}</span>{props.item.source === "custom" && <span>Device-local</span>}</div><h1>{props.item.title}</h1><p>{props.item.summary}</p></div><div className="problem-actions"><button className={favorite ? "favorite active" : "favorite"} onClick={props.onFavorite} aria-label={favorite ? "Remove favorite" : "Add favorite"}>{favorite ? "★" : "☆"}</button>{prompt && <a className="outline-button" href={prompt} target="_blank" rel="noreferrer">Open prompt ↗</a>}</div></div>
      <div className="insight-grid"><article><span className="card-icon">⌁</span><div><small>Pattern cue</small><p>{props.item.cue}</p></div></article><article><span className="card-icon">∞</span><div><small>Invariant</small><p>{props.item.invariant}</p></div></article><article><span className="card-icon">{LANGUAGE_META[props.item.language].short}</span><div><small>{LANGUAGE_META[props.item.language].note}</small><p>{props.item.languageNote}</p></div></article></div>
      ...
      <div className="practice-notes"><article><small>{props.item.track === "ios" ? "Behavior / tradeoff" : "Complexity check"}</small><strong>{props.item.complexity}</strong></article><article><small>{props.item.track === "ios" ? "Recall check" : "Ownership rule"}</small><strong>{props.item.track === "ios" ? props.item.recallChecks?.[Math.min(2, Math.max(0, props.stage - 2))] ?? "Explain the API boundary before typing." : "95%+ accuracy, no peeks. Stage 5 proves independent recall; other passes build syntax."}</strong></article>{Object.keys(props.errorKeys).length > 0 && <article><small>Recent friction</small><strong>{Object.entries(props.errorKeys).sort((a, b) => b[1] - a[1]).slice(0, 4).map(([key, count]) => `${key} ×${count}`).join(" · ")}</strong></article>}</div>{props.state.settings.showKeyboard && <KeyboardGuide errors={props.errorKeys} />}
    </section>
  </main>;
}
...
function SessionsView({ state, items, onStart, onResume, onSkip, onEnd }: { state: AppState; items: PracticeItem[]; onStart: (options: SessionBuildOptions, entries?: SessionQueueEntry[]) => void; onResume: () => void; onSkip: () => void; onEnd: () => void }) {
  const [name, setName] = useState("Focused interview set"); const [count, setCount] = useState(5); const [source, setSource] = useState<SessionSource>("mixed"); const [track, setTrack] = useState<SessionTrack>("interview"); const [language, setLanguage] = useState<SessionLanguage>(state.settings.preferredLanguage); const [pattern, setPattern] = useState<string>("All"); const [difficulty, setDifficulty] = useState<string>("All"); const [stageMode, setStageMode] = useState<SessionStageMode>("recommended");
  ...
  const preview = useMemo(() => buildSessionQueue(items, signals, { count, source, track, language, pattern, difficulty, stageMode }, () => 0.5), [items, signals, count, source, track, language, pattern, difficulty, stageMode]);
  ...
  return <main className="page-container sessions-page"><PageHeading eyebrow="Deliberate practice" title="Build a session that has a finish line." copy="Create a persistent queue, lock in the recall stage for every item, and come back later without losing your place." />
    ...
    <section className="session-builder"><div className="session-form"><span className="eyebrow">Session builder</span><h2>{active ? "Plan the next set" : "Choose the work"}</h2><label><span>Session name</span><input maxLength={80} value={name} onChange={(event) => setName(event.target.value)} /></label><div className="form-pair"><label><span>Track</span><select value={track} onChange={(event) => { const next = event.target.value as SessionTrack; setTrack(next); if (next === "ios") setLanguage("swift"); setPattern("All"); }}><option value="all">All practice</option><option value="interview">Coding interviews</option><option value="ios">iOS &amp; Swift fundamentals</option></select></label><label><span>Language</span><select value={language} disabled={track === "ios"} onChange={(event) => { setLanguage(event.target.value as SessionLanguage); setPattern("All"); }}><option value="all">Python + Swift</option><option value="python">Python</option><option value="swift">Swift</option></select></label></div><div className="form-pair"><label><span>Source</span><select value={source} onChange={(event) => setSource(event.target.value as SessionSource)}><option value="mixed">Smart mix · due first</option><option value="due">Due review only</option><option value="new">New items only</option><option value="favorites">Favorites</option><option value="custom">My snippets</option></select></label><label><span>Number of items</span><select value={count} onChange={(event) => setCount(Number(event.target.value))}>{[3, 5, 8, 10, 15, 20].map((value) => <option key={value} value={value}>{value}</option>)}</select></label></div><div className="form-pair"><label><span>Pattern</span><select value={pattern} onChange={(event) => setPattern(event.target.value)}><option>All</option>{(track === "ios" ? IOS_PATTERN_ORDER : language === "python" ? PYTHON_PATTERN_ORDER : language === "swift" ? INTERVIEW_PATTERN_ORDER : PATTERN_ORDER).map((value) => <option key={value}>{value}</option>)}</select></label><label><span>Difficulty</span><select value={difficulty} onChange={(event) => setDifficulty(event.target.value)}><option>All</option><option>Easy</option><option>Medium</option></select></label></div><label><span>Recall policy</span><select value={stageMode} onChange={(event) => setStageMode(event.target.value as SessionStageMode)}><option value="recommended">Recommended next stage</option><option value="recall">Blank editor for every item</option></select></label><button className="primary-button" disabled={!preview.length} onClick={() => onStart({ name, count, source, track, language, pattern, difficulty, stageMode }, preview)}>{active ? "Replace active session" : "Start session"} · {preview.length} item{preview.length === 1 ? "" : "s"} →</button></div><div className="session-plan"><div className="section-head"><div><small>Queue preview</small><h2>{preview.length ? `${preview.length} selected` : "No matching items"}</h2></div><span>Stages lock when started</span></div><div className="session-preview-list">{preview.map((entry, index) => { const item = items.find((candidate) => candidate.itemId === entry.itemId); return <article key={`${entry.itemId}-${index}`}><span>{index + 1}</span><div><strong>{item?.title}</strong><small>{item ? laneLabel(item) : "Unavailable"} · {item?.pattern} · Stage {entry.stage}</small></div></article>; })}</div>{!preview.length && <p className="session-empty">Broaden the track, language, source, or filters to create this queue.</p>}</div></section>
...
function LibraryView({ state, items, onOpen, onFavorite, onCreate, onEdit, onArchive }: { state: AppState; items: PracticeItem[]; onOpen: (item: PracticeItem, stage?: number) => void; onFavorite: (id: ItemId) => void; onCreate: () => void; onEdit: (item: PracticeItem) => void; onArchive: (id: ItemId) => void }) {
  const [query, setQuery] = useState(""); const [lane, setLane] = useState<"All" | "python" | "swift" | "ios">("All"); const [pattern, setPattern] = useState<Pattern | "All">("All"); const [difficulty, setDifficulty] = useState<Difficulty | "All">("All"); const [status, setStatus] = useState<"All" | "New" | "Learning" | "Owned" | "Due" | "Favorites" | "My snippets">("All"); const [sort, setSort] = useState<Sort>("recommended");
  useEffect(() => {
    function syncLane() {
      const route = parseRoute(window.location.href);
      setLane(route.track === "ios" ? "ios" : route.language === "python" || route.language === "swift" ? route.language : "All");
      setPattern("All");
    }
    ...
    function syncClick(event: Event) {
      ...
      if (index >= 0) {
        ...
        const route: AppRoute = value === "All" ? { view: "library" } : value === "ios" ? { view: "library", language: "swift", track: "ios" } : { view: "library", language: value, track: "interview" };
        window.history.pushState({}, "", serializeRoute(route, window.location.href));
      }
    }
    syncLane(); window.addEventListener("popstate", syncLane); tabs?.addEventListener("click", syncClick);
    return () => { window.removeEventListener("popstate", syncLane); tabs?.removeEventListener("click", syncClick); };
  }, []);
  const filtered = useMemo(() => items.filter((item) => { const stats = itemStats(state, item.itemId); const text = `${itemDisplayId(item)} ${item.title} ${item.pattern} ${item.cue} ${item.tags.join(" ")}`.toLowerCase().includes(query.toLowerCase()); const statusMatch = status === "All" || (status === "New" && !stats.completions) || (status === "Learning" && stats.highestStage > 0 && !stats.owned) || (status === "Owned" && stats.owned) || (status === "Due" && isReviewDue(state, item.itemId)) || (status === "Favorites" && state.favorites.includes(item.itemId)) || (status === "My snippets" && item.source === "custom"); return text && matchesLane(item, lane) && (pattern === "All" || item.pattern === pattern) && (difficulty === "All" || item.difficulty === difficulty) && statusMatch; }).sort((a, b) => sort === "number" ? a.id - b.id : sort === "title" ? a.title.localeCompare(b.title) : sort === "difficulty" ? a.difficulty.localeCompare(b.difficulty) : itemStats(state, a.itemId).highestStage - itemStats(state, b.itemId).highestStage || a.title.localeCompare(b.title)), [items, state, query, lane, pattern, difficulty, status, sort]);
  return <main className="page-container"><div className="heading-actions"><PageHeading eyebrow="Python, Swift, and iOS catalog" title="Choose what to own next." copy="Python interview fluency, Swift algorithms, and practical iOS fundamentals share one progressive recall ladder." /><button className="primary-button" onClick={onCreate}>+ Add code snippet</button></div><div className="track-tabs four" aria-label="Curriculum lane">{(["All", "python", "swift", "ios"] as const).map((value) => <button key={value} aria-pressed={lane === value} className={lane === value ? "active" : ""} onClick={() => { setLane(value); setPattern("All"); }}>{value === "All" ? "All practice" : value === "ios" ? "iOS & Swift" : `${LANGUAGE_META[value].label} interview`}<small>{items.filter((item) => matchesLane(item, value)).length}</small></button>)}</div><div className="library-toolbar"><label className="search-box wide"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search ${items.length} items, patterns, or cues`} /></label><select value={pattern} onChange={(event) => setPattern(event.target.value as Pattern | "All")}><option>All</option>{(lane === "python" ? PYTHON_PATTERN_ORDER : lane === "swift" ? INTERVIEW_PATTERN_ORDER : lane === "ios" ? IOS_PATTERN_ORDER : PATTERN_ORDER).map((value) => <option key={value}>{value}</option>)}</select><select value={difficulty} onChange={(event) => setDifficulty(event.target.value as Difficulty | "All")}><option>All</option><option>Easy</option><option>Medium</option></select><select value={sort} onChange={(event) => setSort(event.target.value as Sort)}><option value="recommended">Recommended</option><option value="number">Catalog order</option><option value="title">Title</option><option value="difficulty">Difficulty</option></select></div>
...
function RecordsView({ state, items, cloud, onOpen, onReview, onToggleUploads, onCloudRefresh }: { state: AppState; items: PracticeItem[]; cloud: CloudRuntime; onOpen: (item: PracticeItem, stage?: number) => void; onReview: () => void; onToggleUploads: (enabled: boolean) => void; onCloudRefresh: () => void }) {
  ...
  const trackCoverage = (["python", "swift", "ios"] as const).map((lane) => { const group = items.filter((item) => matchesLane(item, lane)); const started = group.filter((item) => itemStats(state, item.itemId).highestStage > 0).length; const trackOwned = group.filter((item) => itemStats(state, item.itemId).owned).length; return { lane, total: group.length, started, owned: trackOwned, percent: group.length ? Math.round((group.reduce((sum, item) => sum + itemStats(state, item.itemId).highestStage, 0) / (group.length * 5)) * 100) : 0 }; });
  const friction = Object.entries(state.attempts.reduce<Record<string, number>>((counts, attempt) => { Object.entries(attempt.keyErrors).forEach(([key, count]) => { counts[key] = (counts[key] ?? 0) + count; }); return counts; }, {})).sort((a, b) => b[1] - a[1]).slice(0, 8); const maxFriction = Math.max(1, ...friction.map(([, count]) => count));
  return <main className="page-container"><PageHeading eyebrow="Local profile + community beta" title="Records you can trust." copy="Your learning history stays local. If you opt in, built-in completed runs can also power a private profile, recent activity, and server-ranked community records." /><CommunityPanel state={state} items={items} status={cloud.status} session={cloud.session} onToggleUploads={onToggleUploads} onRefresh={onCloudRefresh} /><div className="stat-grid"><StatCard label="Completed passes" value={String(attempts.length)} note={`${currentEligible.length} current-revision records`} /><StatCard label="Eligible speed" value={`${avgWpm} WPM`} note={`${avgAccuracy}% average across all passes`} /><StatCard label="Current streak" value={`${activeStreak(state)} days`} note={`${practicedMinutesToday(state)} minutes today`} /><StatCard label="Owned solutions" value={`${owned}/${items.length}`} note="Clean blank-editor recall" /></div>
    ...
    <div className="dashboard-grid"><section className="dashboard-card"><div className="section-head"><div><small>Three learning lanes</small><h2>Track coverage</h2></div><span>Independent recall</span></div><div className="track-coverage">{trackCoverage.map((value) => <article key={value.lane}><span><strong>{value.lane === "ios" ? "iOS & Swift fundamentals" : `${LANGUAGE_META[value.lane].label} interviews`}</strong><small>{value.started} started · {value.owned} owned · {value.total} total</small></span><div><i style={{ width: `${value.percent}%` }} /></div><b>{value.percent}%</b></article>)}</div></section><section className="dashboard-card"><div className="section-head"><div><small>Across every attempt</small><h2>Key friction</h2></div><span>Persisted misses</span></div>{friction.length ? <div className="friction-list">{friction.map(([key, count]) => <div key={key}><span><kbd>{key === " " ? "space" : key}</kbd><small>{count} misses</small></span><i><b style={{ width: `${(count / maxFriction) * 100}%` }} /></i></div>)}</div> : <div className="empty-history">Mistyped keys will accumulate here after practice.</div>}</section></div>
    <section className="dashboard-card records-card"><div className="section-head"><div><small>Qualified only</small><h2>Personal bests</h2></div><span>Exact item · stage · mode</span></div>{bests.length ? <div className="records-grid">{bests.map((attempt) => <article key={attempt.id}><span><small>{LANGUAGE_META[attempt.language].label} · {attempt.mode} · stage {attempt.stage}</small><strong>{attempt.titleSnapshot}</strong></span><b>{attempt.wpm}<small> WPM</small></b><em>{attempt.accuracy}%</em></article>)}</div> : <div className="empty-history">Complete a 95%+ no-peek pass to set your first personal best.</div>}</section>
...
function CustomSnippetDialog({ item, onClose, onSave }: { item?: PracticeItem; onClose: () => void; onSave: (input: Parameters<typeof makeCustomItem>[0]) => void }) {
  const [title, setTitle] = useState(item?.title ?? ""); const [track, setTrack] = useState<"interview" | "ios">(item?.track ?? "interview"); const [language, setLanguage] = useState<CodeLanguage>(item?.language ?? "python"); const [pattern, setPattern] = useState<Pattern>(item?.pattern ?? PYTHON_PATTERN_ORDER[0]); const [difficulty, setDifficulty] = useState<"Easy" | "Medium">(item?.difficulty ?? "Easy"); const [code, setCode] = useState(item?.code ?? "def example(values: list[int]) -> list[int]:\n    # Type your Python implementation here\n    return values"); const [cue, setCue] = useState(item?.cue ?? ""); const [invariant, setInvariant] = useState(item?.invariant ?? ""); const [complexity, setComplexity] = useState(item?.complexity ?? ""); const [languageNote, setLanguageNote] = useState(item?.languageNote ?? ""); const valid = title.trim().length >= 1 && title.trim().length <= 80 && code.trim().length >= 10 && code.length <= 20000;
  const dialogRef = useRef<HTMLElement>(null); useModalKeyboard(onClose, dialogRef);
  const patterns = track === "ios" ? IOS_PATTERN_ORDER : language === "python" ? PYTHON_PATTERN_ORDER : INTERVIEW_PATTERN_ORDER;
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section ref={dialogRef} className="custom-dialog" role="dialog" aria-modal="true" aria-labelledby="custom-title"><button className="dialog-close" onClick={onClose} aria-label="Close">×</button><span className="eyebrow">Device-local curriculum{item ? ` · revision ${item.contentRevision}` : ""}</span><h2 id="custom-title">{item ? "Edit code snippet" : "Add a code snippet"}</h2><p>{item ? "Metadata edits preserve mastery. Changing code creates a new revision while keeping the complete attempt history." : "Turn a Python solution, Swift algorithm, or iOS pattern into the same progressive recall exercise."}</p><div className="custom-form"><label><span>Title</span><input data-modal-autofocus maxLength={80} value={title} onChange={(event) => setTitle(event.target.value)} placeholder="e.g. Top-k with a min heap" /></label><div className="form-pair"><label><span>Track</span><select value={track} onChange={(event) => { const next = event.target.value as "interview" | "ios"; setTrack(next); if (next === "ios") { setLanguage("swift"); setPattern(IOS_PATTERN_ORDER[0]); } else setPattern(language === "python" ? PYTHON_PATTERN_ORDER[0] : INTERVIEW_PATTERN_ORDER[0]); }}><option value="interview">Coding interviews</option><option value="ios">iOS &amp; Swift fundamentals</option></select></label><label><span>Language</span><select value={language} disabled={track === "ios"} onChange={(event) => { const next = event.target.value as CodeLanguage; setLanguage(next); setPattern(next === "python" ? PYTHON_PATTERN_ORDER[0] : INTERVIEW_PATTERN_ORDER[0]); if (!item) setCode(next === "python" ? "def example(values: list[int]) -> list[int]:\n    # Type your Python implementation here\n    return values" : "func example(_ values: [Int]) -> [Int] {\n    // Type your Swift implementation here\n    values\n}"); }}><option value="python">Python</option><option value="swift">Swift</option></select></label></div><div className="form-pair"><label><span>Pattern</span><select value={pattern} onChange={(event) => setPattern(event.target.value as Pattern)}>{patterns.map((value) => <option key={value}>{value}</option>)}</select></label><label><span>Difficulty</span><select value={difficulty} onChange={(event) => setDifficulty(event.target.value as "Easy" | "Medium")}><option>Easy</option><option>Medium</option></select></label></div><label><span>{LANGUAGE_META[language].label} code</span><textarea value={code} onChange={(event) => setCode(event.target.value)} spellCheck={false} /></label><label><span>Pattern cue</span><input value={cue} onChange={(event) => setCue(event.target.value)} placeholder="What should you recognize before coding?" /></label><label><span>Invariant</span><input value={invariant} onChange={(event) => setInvariant(event.target.value)} placeholder="What must remain true?" /></label><div className="form-pair"><label><span>Complexity or tradeoff</span><input value={complexity} onChange={(event) => setComplexity(event.target.value)} placeholder="Behavior, cost, or lifecycle tradeoff" /></label><label><span>{LANGUAGE_META[language].note}</span><input value={languageNote} onChange={(event) => setLanguageNote(event.target.value)} placeholder="Syntax or API detail to remember" /></label></div></div><div className="result-actions"><button className="outline-button" onClick={onClose}>Cancel</button><button className="primary-button" disabled={!valid} onClick={() => onSave({ title, track, language, pattern, difficulty, code, cue, invariant, complexity, languageNote })}>{item ? "Save changes" : "Save and practice"} →</button></div></section></div>;
}
...
function Segmented({ value, options, onChange }: { value: string; options: string[]; onChange: (value: string) => void }) { return <div className="segmented">{options.map((option) => <button className={value === option ? "active" : ""} aria-pressed={value === option} onClick={() => onChange(option)} key={option}>{/^[a-z]/.test(option) ? option[0].toUpperCase() + option.slice(1) : option}</button>)}</div>; }
