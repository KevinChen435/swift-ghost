...
function TodayView({ ready, state, items, onOpen, onReview, onBrowse, onCreate, onSessions }: { ready: boolean; state: AppState; items: PracticeItem[]; onOpen: (item: PracticeItem, stage?: number, challengeDate?: string, sessionId?: string) => void; onReview: () => void; onBrowse: () => void; onCreate: () => void; onSessions: () => void }) {
  const todayDate = ready ? new Date() : new Date(2000, 0, 1, 12); const today = dayKey(todayDate); const interviewItems = BUILTIN_ITEMS.filter((item) => item.track === "interview"); const iosItems = BUILTIN_ITEMS.filter((item) => item.track === "ios"); const daily = dailyItem(interviewItems, todayDate); const iosDaily = dailyItem(iosItems, new Date(todayDate.getTime() + 86400000)); const due = items.filter((item) => isReviewDue(state, item.itemId));
