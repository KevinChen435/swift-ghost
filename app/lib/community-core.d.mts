...
export function validateAttemptUpload(
  raw: unknown,
  now?: number,
): { ok: true; value: ValidatedAttempt } | { ok: false; error: string };
export function rankItemRows(
  rows: Array<{
    displayName: string | null;
    itemRevision: number;
    stage: number;
...
export function rankDailyRows(
  rows: Array<{
    displayName: string | null;
    completions: number;
    averageAccuracyBps: number;
    wpmBps: number;
...
export function isCurrentDailyChallenge(
  challenge: unknown,
  items: Array<{
    itemId: string;
    itemRevision: number;
