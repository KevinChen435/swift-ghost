import { deliverCallback, deliverExecutionCallback } from "./callback";
import { judgeSubmission } from "./core";
import { judgeExecution } from "./execution";
import { parseExecution, parsePositiveInt, parseSubmission } from "./schema";
  ...
  for (const message of batch.messages) {
    try {
      ...
      if (body.kind === "execution") {
        const request = parseExecution(body.request, env.CALLBACK_ALLOWED_ORIGINS);
        const result = await judgeExecution(request, sandboxFactory, {
          timeoutMs: parsePositiveInt(env.TEST_TIMEOUT_MS, 4_000, 100, 30_000),
          compileTimeoutMs: parsePositiveInt(env.COMPILE_TIMEOUT_MS, 20_000, 1_000, 60_000),
          outputLimitBytes: parsePositiveInt(env.OUTPUT_LIMIT_BYTES, 65_536, 1_024, 262_144),
        });
        await env.JUDGE_QUEUE.send(
          { kind: "execution-callback", callbackUrl: request.callbackUrl, result },
          { contentType: "json" },
        );
        message.ack();
        continue;
      ...
      if (body.kind === "execution-callback") {
        await deliverExecutionCallback(body, env);
        message.ack();
        continue;
      }
