...
- A three-lane Test Design Academy with 24 original Python, Swift, and iOS contract probes across 12 skills; reload-safe committed drafts; explicit baseline/boundary/adversarial/regression purposes; conservative reference-oracle confirmation that never marks novel cases wrong; delayed distinct-probe retention; lane-aware blank solve or concept-recall handoffs; and boundary/verification evidence routed to Weakness Lab without pretending designed cases were executed, compiled, or sent anywhere
- A first-class Swift/iOS Concept Recall lane: answer privately, commit before reveal, compare against 48 authored reference answers, optionally type over the grey answer, then self-grade without automated semantic scoring; portable companion cards can graduate into isolated Swift solving
...
- A unified Weakness Lab that turns recurring debrief, solution-review, assessment, mock, transfer, and Test Design signals into ranked Python, Swift, and iOS remediation cases, targeted three-context practice queues, exact evidence trails, Python transfer proof, and distinct delayed Swift/iOS context proof
...
- Durable Challenge Sets that freeze a selected catalog run, preserve revisions and launch provenance, resume eligible work, and expose a private Activity ledger without turning completion into mastery
...
- Separate fail-closed Verified Python and Swift checkpoint lanes with server-selected prompt revisions, transient source upload, immutable pending receipts, signed asynchronous settlement, aggregate-only sealed results, Swift public-example execution, and execution-only Swift custom cases with bounded observed output; each appears as unavailable until a VM-backed judge gateway is explicitly connected
...
The GitHub Pages edition is entirely local: it has no account requirement, telemetry, cookies, or network-backed progress. Pattern Academy retrieval responses, Core Pattern Skill Check history and due dates, frozen Cross-lane Re-entry forms and reports, Test Design drafts/history/due dates, Fluency Clinic cases and reconstruction evidence, practice history, interview notebooks, virtual-round reports, source snapshots, solution-review explanations and notes, rubric responses, and settings stay in the browser's local storage, and a portable JSON export is available in Settings. Test Design reference cases are original teaching examples, not hidden judge payloads; learner cases are design-only evidence and are not executed, compiled, copied into solves, or sent to a server. Python checks execute only inside an isolated browser worker and never send source code to a server. Swift reconstruction uses exact keystroke comparison and does not compile Swift or evaluate alternate implementations; the portable Swift solve lane is a separate authenticated server path, while Swift/iOS concept recall remains self-assessed. The fast local interpreter covers every shipped exercise and common interview helpers, but it is intentionally labeled as a subset: CPython-only packages and some newer Python language features are outside its scope.

Browser persistence is designed for one active tab per profile. Concurrent edits to the same profile in multiple tabs are unsupported and resolve last-write-wins; bounded append-only attempt records are deduplicated by ID during normalization, and portable v35 backups remain the recovery path.

The hosted community edition uses ChatGPT identity. Signed-in learners can privately sync bounded Study Plan structure, collection membership, and plan/session links between devices. Private Progress Sync is a separate opt-in: it carries only normalized built-in attempt summaries and learning-event metadata, with server-owned revisions. Favorites remain local until a deletion-aware preference sync exists. Code, transcripts, notebooks, custom testcases, solution-review records, reference answers, and custom practice-item contents stay local and are excluded from both sync paths. Community uploads are off by default and must be explicitly enabled. Only completed built-in attempt summaries can upload; custom challenges and snippets, prompts, judge cases, drafts, source code, email addresses, and key-level telemetry are never public. Public profiles, activity sharing, and leaderboard participation each require a separate opt-in.

The optional Verified Python and Swift lanes are the explicit exceptions to local source handling: pressing a sealed-test Submit button uploads that checkpoint source to the configured judge, while Swift's Run examples and custom-case rehearsal actions send the current source to separate isolated execution namespaces. Custom rehearsal accepts only caller-entered arguments, returns bounded observed output, never receives hidden or expected values, and is never counted as evidence or assignment acceptance. The application keeps a transient retry copy only until the gateway durably accepts the queued job (or until the signed result settles) and persists only a source hash plus bounded aggregate receipt. Browser-local assessments and ordinary practice never silently use these paths or inherit their verified labels.
