...
- 20 server-judged portable Swift solve contracts, including two iOS-lane companion cards for value semantics and optional binding; public examples are runnable while sealed cases remain server-only
...
- Separate fail-closed Verified Python and Swift checkpoint lanes with server-selected prompt revisions, transient source upload, immutable pending receipts, signed asynchronous settlement, aggregate-only sealed results, and Swift public-example execution; each appears as unavailable until a VM-backed judge gateway is explicitly connected
...
The optional Verified Python and Swift lanes are the explicit exceptions to local source handling: pressing a sealed-test Submit button uploads that checkpoint source to the configured judge, while Swift's Run examples action sends only the current source to the public-sample namespace. The application keeps a transient retry copy only until the gateway durably accepts the queued job (or until the signed result settles) and persists only a source hash plus bounded aggregate receipt. Browser-local assessments and ordinary practice never silently use these paths or inherit their verified labels.
...
The local-first training core remains the priority. Planned work includes richer boundary suites, authored semantic masks, Swift custom-case rehearsal, server-assigned private transfer judging, broader server-backed Python verification, and private cross-device sync for learning evidence beyond the current Study Plan structure.
