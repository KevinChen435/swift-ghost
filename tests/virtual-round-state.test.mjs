...
test("state v23 preserves virtual rounds after the complete v22 fallback", async () => {
  ...
  assert.match(product, /export const STORAGE_KEY = "swift-ghost-state-v29"/);
...
test("backup copy advertises the v29 envelope and all older supported imports", async () => {
  const app = await readFile(
    new URL("../app/components/SwiftGhostApp.tsx", import.meta.url),
    "utf8",
  );
  ...
  assert.match(app, /portable v29 backup envelope/);
  assert.match(app, /supported v2-v29 backups/);
