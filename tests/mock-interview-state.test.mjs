...
test("state v17 preserves multi-problem mock notebooks and debriefs", async () => {
  ...
  assert.match(product, /export const STORAGE_KEY = "swift-ghost-state-v32"/);
  ...
  assert.match(
    product,
    /STATE_STORAGE_KEYS = \[\s+STORAGE_KEY,\s+THIRTY_FIRST_STORAGE_KEY,\s+THIRTIETH_STORAGE_KEY,\s+TWENTY_NINTH_STORAGE_KEY/,
  );
