...
test("state v21 persists Interview Studio and migrates all older state versions", async () => {
  ...
  assert.match(
    product,
    /STATE_STORAGE_KEYS = \[\s+STORAGE_KEY,\s+TWENTIETH_STORAGE_KEY,\s+NINETEENTH_STORAGE_KEY,\s+EIGHTEENTH_STORAGE_KEY,\s+SEVENTEENTH_STORAGE_KEY,\s+SIXTEENTH_STORAGE_KEY,\s+FIFTEENTH_STORAGE_KEY/,
  );
});
